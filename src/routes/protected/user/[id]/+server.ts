...
export async function GET({ locals, params }) {
  ...
  try {
    ...
    return new Response(JSON.stringify({
      message: 'User profile', completedProjectCount, sharedProjects: sharedProjects.map(project => ({
        ...project,
        userCount: project.users.length, // Add user count for each project
      })), completedTaskCount, relatedTaskCount, userProfile, relatedProjectCount, user, possibleConvo
    }), { status: 200 });
  } catch (error) {
