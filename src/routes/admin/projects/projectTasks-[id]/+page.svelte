<script lang="ts">
	...
	async function saveTasks() {
		...
		const response = await fetch(`/admin/projects/projectTasks-${projectId}`, {
			method: 'POST',
			headers: {
...
<div class="lg:w-4/5 md:w-5/6 mx-auto py-10">
	<div>
			{#if loading}
			<Icon icon="line-md:loading-twotone-loop" width="24" height="24" />
			{:else}
			<h1 class="px-5 text-3xl font-bold"><span class="text-lg">Project: </span>{project?.title}</h1>
			{/if}
	</div>
